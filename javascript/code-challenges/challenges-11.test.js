...
------------------------------------------------------------------------------------------------ */
// add the zero so that this will work with empty arrays
const count = (target, input) => {
  return input
    .map (arr => arr.filter (x => x === target))
    .reduce ((element, current) => element + current.length, 0);
};
...
const divisibleByFiveTwoToThePower = input => {
  return input.map(arr=>arr.filter(
    n=>(typeof n)==='number' && (n%5===0)).map(
    n=>Math.pow(2,n)));
...
let findMaleAndFemale = data => {
  return data.filter(c => /(fe)?male$/.test(c.gender)).map(
    c=>c.name).join(' and ');
};
...
let findShortest = data => {
  return data.reduce((person1, person2)=>Number(person1.height)< Number(person2.height) ? person1 : person2).name;
};
...
describe ('Testing challenge 2', () => {
  test ('It should return the number of times the input is in the nested arrays', () => {
    expect (count (5, [[1, 3, 5, 7, 9], [5, 5, 5], [1, 2, 3]])).toStrictEqual (
      4
    );
    expect (count (3, [[1, 3, 5, 7, 9], [5, 5, 5], [1, 2, 3]])).toStrictEqual (
      2
    );
    expect (count (12, [[1, 3, 5, 7, 9], [5, 5, 5], [1, 2, 3]])).toStrictEqual (
      0
    );
  });
