...
const addTwo = (arr) => {
  //create answer array
  let answer = [];
  //using a for loop, loop through each element +2, push into answer array
  for(let i of arr){
    answer.push(i+2);
  }
  //return answer array
...
const typeNum = (arr) => {
  //write a function that checks each element to see if its a number
  let num = (val)=>{
    if(typeof(val)==='number'){
      return val;
    }
  }
  //use filter to call num fucntion
...
const containsAnd = (arr) => {
  //filter out words that dont contain and , push remaining into new array
  let answer = arr.filter(word => word.includes('and'))
  //return answer array
  return answer;
};
...
//alternative answer
//const oddValues = (arr){
//let answer = arr.filter(num => num %2);
//return answer;
//};
...
const notInFirstArray = (forbiddenValues, arr) => {
  //the exlamation point makes it so that it returns the values not in the original array(!=opposite)
