...
const addOne = (arr) => {
  let answer = [];
//use forEach to loop thorugh the array
  arr.forEach(num => answer.push(num +1));
//push the value +1 into a new array
...
const addExclamation = (arr) => {
  ...
  let answer = [];
  // push the value with an ! at the end of each
...
const allUpperCase = (arr) => {
  ...
  let answer = [];
  // use allUppercase to convert elements into upper case
...
const greeting = (word) => {
  // take a string and return it with capital letters and !
  let answer = word.toUpperCase()+'!';
  return answer;
};

const speaker = (words, callback) => {
  //use forEach to build a 
  let answer = [];
...
const createList = (availableItems) => {
  //set answer array
  let answer= [];
  //loop through items & check if they are available
  availableItems.forEach(item => {
    //for all available items add them into our empty array
    if(item.available){
